function [results] = function_max_entropy_optimized(sim_param)
    % function_max_entropy_optimized: 该函数用于优化并模拟基于最大熵原理的检测过程，考虑了诚实节点和拜占庭节点的行为。
    %
    % 输入参数:
    ...
    N = sim_param.N;           % 节点总数
    % M = sim_param.M;         % 拜占庭节点数量（未使用）
    % K = sim_param.K;         % 诚实节点数量（未使用）
    % alfa = sim_param.alfa;   % 拜占庭节点的比例（未使用）
    ...
    M_all = gen_rd(N, Nprove); % 生成每个仿真中拜占庭节点的数量（加权分布）
